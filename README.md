# Lendico Payment Plan Generator
...
3. Execute `mvn clean install` This will build the project by downloading all the dependencies. And create jar file inside target folder.
4. Execute `mvn clean spring-boot:run`. If port 8080 is in use, replace the port number in `application.properties`.
5. Open http://localhost:8080/swagger-ui.html in the browser. it is a Swagger for API documentation. Here you can try out the APIs request and response


### Example payload / request body
Please use the below request in the Swagger UI.
```json
{
	"loanAmount": "5000",
	"nominalRate": "5.0",
